...
/datum/async_map_generator/space_ruin/proc/post_generation()
	...
	for(var/door_pos in placed_room_entrances)
		...
		if(valid)
			...
			switch(placed_room_entrances[door_pos])
				if(SOUTH, NORTH)
					var/obj/machinery/atmospherics/pipe/simple/supply/hidden/layer2/b1 = new(T)
					var/obj/machinery/door/firedoor/border_only/b2 = new(T)
					var/obj/effect/turf_decal/stripes/closeup/b3 = new(T)
					b1.setDir(NORTH)
					b2.setDir(NORTH)
					b3.setDir(2)
				if(EAST, WEST)
					var/obj/machinery/atmospherics/pipe/simple/supply/hidden/layer2/b1 = new(T)
					var/obj/machinery/atmospherics/pipe/simple/scrubbers/hidden/layer4/b2 = new(T)
					var/obj/effect/turf_decal/stripes/closeup/b3 = new(T)
					b1.setDir(EAST)
					b2.setDir(EAST)
					b3.setDir(1)
